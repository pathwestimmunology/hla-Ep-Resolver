   ...
   observeEvent(input$submit, {
         ...
         suppressMessages(
            mfi_data() %>%
               ...
               mutate(
                  ...
                  Threshold = ifelse(as.numeric(Num) >= 4000, "PASS", "FAIL")
               ) %>%
               dplyr::select(-Num) %>%
               mutate(
      ...
      eplets_match <- reactive({
         ...
         Serotype <- bind_rows(x, y) %>%
            filter(!is.na(MRO_Serotype)) %>%
            distinct() %>%
            group_by(OrganMatch.2nd.F) %>%
            mutate(MRO_Serotype = paste(unique(MRO_Serotype), collapse = ", ")) %>%
            slice(1) %>%
            ungroup() %>%
            mutate(MRO_Serotype = sapply(
               strsplit(MRO_Serotype, ","), function(x) {
         ...
         process_cwd_data <- function(mfi_data) {
            df <- mfi_data %>%
               ...
               group_by(key) %>%
               summarize(Candidate_antigen = paste(unique(Candidate_antigen), collapse = ", ")) %>%
               ungroup() %>%
         ...
         df <- lapply(1:nrow(combined.cwd.cleanOrganMatch), function(i)  {
      ...
      eplets_match_geno <- reactive({
         ...
         df.geno_allele1 <- lapply(1:nrow(combine_bead_patient_typing), function(i)  {
         ...
         df.geno_allele1 <- do.call(rbind, df.geno_allele1)

         eplets_shared_geno <- data.frame()

         df.geno_allele2 <- lapply(1:nrow(combine_bead_patient_typing), function(i)  {
            ## Get patient and organ match allele from row
            row <- combine_bead_patient_typing[i, ]
         ...
         df.geno_allele2 <- do.call(rbind, df.geno_allele2)
      ...
      eplets_donor_of_interest <- reactive({
         ...
         Serotype.d <- bind_rows(a, b) %>%
            filter(!is.na(MRO_Serotype)) %>%  # Changed na.omit() to filter()
            distinct() %>%
            group_by(OrganMatch.2nd.F) %>%
            mutate(MRO_Serotype = paste(unique(MRO_Serotype), collapse = ", ")) %>%
            slice(1) %>%
         ...
         df_doi <- lapply(1:nrow(organ_match_donor), function(i)  {
      ...
      eplets_match_geno_doi <- reactive({
         ...
         doi.df.geno_allele1 <- lapply(1:nrow(combine_doi_patient_typing), function(i)  {
            ## Get patient and organ match allele from row
         ...
         doi.df.geno_allele1 <- do.call(rbind, doi.df.geno_allele1)

         eplets_shared_geno <- data.frame()

         doi.df.geno_allele2 <- lapply(1:nrow(combine_doi_patient_typing), function(i)  {
            ## Get patient and organ match allele from row
            row <- combine_doi_patient_typing[i, ]
            patient.allele2 <- stringr::str_trim(row[["allele_2"]], side = c("both"))
            doi_allele <- stringr::str_trim(row[["DonorOfInterest.2nd.F"]], side = c("both"))
         ...
         doi.df.geno_allele2 <- do.call(rbind, doi.df.geno_allele2)
      ...
      shinyalert::shinyalert(
         ...
         text = '<div style="color: black; text-align: justify;">
            ...
            <ul style="list-style-type: square; color: blue; text-align: justify;">
               ...
               <li>determines whether these identified alleles have shared eplets or serotypes with the patient’s high-risk anti-HLA specificities (with an MFI exceeding 4000 in strength),</li>
